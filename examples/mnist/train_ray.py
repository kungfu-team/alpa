...
def get_train_data_laoder(train_ds, state, batch_size):
    ...
    batch_images = jax.core.ShapedArray((batch_size, 28, 28, 1), jnp.float32)
    batch_labels = jax.core.ShapedArray((batch_size,), jnp.int32)
    ...
    data_loader = alpa.MeshDriverDataLoader(
        ...
        len(images_np),
        input_iter_func,
        executable.get_input_placement_specs()[1:3],
        prefetch_size=4,
        repeat=True)
...
def train_and_evaluate(config: ml_collections.ConfigDict,
  ...
  """
    ...
    # Load state
    #  with open("/data/start.pickle", "rb") as fi:
    #      state_dict = pickle.load(fi)
    ...
    for epoch in range(1, config.num_epochs + 1):
        ...
        test_loss, test_accuracy = eval_step(state, test_ds['image'],
                                             test_ds['label'])
        ...
        state_dict = to_np_arr(state_dict)
        with open(f"/data/state_{epoch}.pickle", "wb") as fi:
    ...
    with open('/data/mnist_hlo.txt', 'w') as fi:
        fi.write(train_step.get_last_executable().get_hlo_text())
